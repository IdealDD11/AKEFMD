...
class KfNNdmd(Edmd):
    ...
    def forward(self,l,Z, Z_dot, U, U_nom,Z_kl, U_kl, U_nom_kl,Lambda_matrix):
        # gt = self.encoder(xt)
        # xt_ = self.decoder(gt)
...
###add batch size
    def sample(self,l,Z):
        ...
        lent_new=lent-l
        z_new=zeros((Z.shape[0],int(Z.shape[1]-tr*l)))
        for i in range(tr):
            z_new[:,int(i*lent_new):int((i+1)*lent_new)]=Z[:,int(i*lent):int(i*lent+lent_new)]
        return z_new                                                                                                                                               

    def addbatch(self,batch_size,Z, Z_dot, U, U_nom,Z_kl, U_kl, U_nom_kl):
        rr=Z_kl.shape[1]-batch_size
        rrr=np.random.randint(0, rr, size=1)
        r=rrr[0]
        Z1=Z[:,r:r+batch_size]
        Z_dot1=Z_dot[:,r:r+batch_size]
        U1=U[:,r:r+batch_size]
        U_nom1=U_nom[:,r:r+batch_size]
        Z_kl1=Z_kl[:,r:r+batch_size]
        U_kl1=U_kl[:,r:r+batch_size]
        U_nom_kl1=U_nom_kl[:,r:r+batch_size]
        # print(Z1.shape,U1.shape,U_nom1.shape,Z_kl1.shape,U_kl1.shape)
               
        return np.array(Z1).reshape(batch_size, -1), np.array(Z_dot1).reshape(batch_size, -1), np.array(U1).reshape(batch_size, 1)  ,    np.array(U_nom1).reshape(batch_size, 1), np.array(Z_kl1).reshape(batch_size, -1), np.array(U_kl1).reshape(batch_size, 1) , np.array(U_nom_kl1).reshape(batch_size, 1)                                                                                                                                            

    
    def train(self, Z, Z_dot, U, U_nom, max_iter=10,lr =0.001):
        mseloss = nn.MSELoss()
        # l1loss = nn.L1Loss()
        l=50
        batchsize=64
        Z_kl, U_kl, U_nom_kl = self.sample(l,Z),self.sample(l,U),self.sample(l,U_nom)
        ...
        for it in range(max_iter):
            ...
            for _ in range(100):
                ...
                Z_batch, Z_dot_batch, U_batch, U_nom_batch,Z_kl_batch, U_kl_batch, U_nom_kl_batch = self.addbatch(batchsize,Z, Z_dot, U, U_nom,Z_kl, U_kl, U_nom_kl)
                Lambda_matrix=diag(self.basis.Lambda)

                Z_batch = torch.FloatTensor(Z_batch)
                Z_dot_batch = torch.FloatTensor(Z_dot_batch)
                U_batch = torch.FloatTensor(U_batch)
                U_nom_batch = torch.FloatTensor(U_nom_batch)
                Z_kl_batch = torch.FloatTensor(Z_kl_batch)
                U_kl_batch = torch.FloatTensor(U_kl_batch)
                U_nom_batch = torch.FloatTensor(U_nom_kl_batch)
                Lambda_matrix = torch.FloatTensor(Lambda_matrix)
                

                Z_dot_hat,Z_kl_hat = self.forward(l,Z_batch, Z_dot_batch, U_batch, U_nom_batch,Z_kl_batch, U_kl_batch, U_nom_kl_batch,Lambda_matrix)

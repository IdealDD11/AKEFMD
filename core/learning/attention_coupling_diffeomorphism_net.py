# -*- coding: utf-8 -*-
"""
Created on Tue Jul 19 15:53:59 2022
...
# class AttentionCouplingDiffeomorphismNet(nn.Module):

#     def __init__(self, n, A_cl, jacobian_penalty = 1e-2, n_hidden_layers = 5, layer_width=50, batch_size = 64, dropout_prob=0.1, traj_input=False):
#         super(AttentionCouplingDiffeomorphismNet, self).__init__()
#         self.n = n
...
#         self.channel_part_1 = self.d_h_in // 2-1
#         self.channel_part_2 = self.d_h_in - self.d_h_in// 2+1
...
#         self.encoder = nn.Sequential(nn.Linear(self.channel_part_1+self.channel_part_2, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, self.channel_part_2).double().to(self.device))
...
#         self.encoder1 = nn.Sequential(nn.Linear(self.channel_part_2, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, self.channel_part_2).double().to(self.device))
#         self.encoder2 = nn.Sequential(nn.Linear(self.channel_part_2, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, self.channel_part_2).double().to(self.device))
#         self.encoder3 = nn.Sequential(nn.Linear(self.channel_part_2, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, self.channel_part_2).double().to(self.device))
#         self.encoder4 = nn.Sequential(nn.Linear(self.channel_part_2, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, self.channel_part_2).double().to(self.device))
#         self.encoder5 = nn.Sequential(nn.Linear(self.channel_part_2, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, self.channel_part_2).double().to(self.device))
#         self.encoder6 = nn.Sequential(nn.Linear(self.channel_part_2, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
#                                       nn.ReLU(),
#                                       nn.Linear(H, H).double().to(self.device),
...
#     def diffJaco(self, xt, input_dimension,output_dimension,encoder): 
#         #########
#         fc_in=encoder[0]
#         fc_hidden = []
#         for _ in range(self.n_hidden_layers):
#             fc_hidden.append(encoder[_+1])
#         # fc_hidden.append(encoder[1])
#         # fc_hidden.append(encoder[2])
#         fc_out=encoder[-1]
#         #########
...
#             # h.append(F.relu(fc_hidden[ii](h[-1])))
#             h.append(fc_hidden[ii](h[-1]))
#         h_out = fc_out(h[-1])

#         # Define diffeomorphism Jacobian model:
#         h_grad = fc_in.weight
#         h_grad = mm(fc_hidden[1].weight, h_grad)
#         h_grad = h_grad.unsqueeze(0).expand(cur_batch_size, self.layer_width, input_dimension)
#         delta = F.relu(fc_hidden[1](h[2])).sign().unsqueeze_(-1).expand(cur_batch_size, self.layer_width,input_dimension)
#         h_grad = delta * h_grad
#         for ii in range(1, self.n_hidden_layers):
#             h_grad = bmm(fc_hidden[ii].weight.unsqueeze(0).expand(cur_batch_size, self.layer_width, self.layer_width), h_grad)
#             delta = F.relu(fc_hidden[ii](h[ii+1])).sign().unsqueeze_(-1).expand(cur_batch_size,self.layer_width,input_dimension)
#             h_grad = delta * h_grad
#         # h_grad = bmm(fc_hidden[3].weight.unsqueeze(0).expand(cur_batch_size, self.layer_width, self.layer_width), h_grad)
#         # delta = F.relu(fc_hidden[3](h[4])).sign().unsqueeze_(-1).expand(cur_batch_size,self.layer_width,input_dimension)
#         # h_grad = delta * h_grad

#         h_grad = bmm(fc_out.weight.unsqueeze(0).expand(cur_batch_size,output_dimension,self.layer_width), h_grad)
#         return h_grad,h_out
    
    
#     def st_network1(self, x, channel_part_1,channel_part_2):
        
#             E_grad,E_out = self.diffJaco(x,self.channel_part_1+self.channel_part_2,self.channel_part_2,self.encoder)
#             N1_grad,N1_out = self.diffJaco(E_out,self.channel_part_2,self.channel_part_2,self.encoder1)
#             N2_grad,N2_out = self.diffJaco(E_out,self.channel_part_2,self.channel_part_2,self.encoder2)
...
#             # print(F1_grad.shape,F1_out.shape)
#             N3_grad,N3_out = self.diffJaco(F1_out,self.channel_part_2,self.channel_part_2,self.encoder3)
...
#     def st_network2(self, x, channel_part_1,channel_part_2):
        
#             E_grad,E_out = self.diffJaco(x,self.channel_part_1+self.channel_part_2,self.channel_part_2,self.encoder)
#             N1_grad,N1_out = self.diffJaco(E_out,self.channel_part_2,self.channel_part_2,self.encoder4)
#             N2_grad,N2_out = self.diffJaco(E_out,self.channel_part_2,self.channel_part_2,self.encoder5)
#             # print(E_grad.shape,E_out.shape,N1_grad.shape,N1_out.shape,N2_grad.shape,N2_out.shape)
#             F1_grad,F1_out =E_grad*N1_out.unsqueeze(-1)+bmm(N2_grad,E_grad)+E_out.unsqueeze(-1)*bmm(N1_grad,E_grad) ,E_out*N1_out+N2_out
#             # print(F1_grad.shape,F1_out.shape)
#             N3_grad,N3_out = self.diffJaco(F1_out,self.channel_part_2,self.channel_part_2,self.encoder6)
#             # F2_grad,F2_out=E_grad+F1_grad*N3_grad+F1_out.unsqueeze(-1)*bmm(N3_grad,F1_grad),  E_out+F1_out*N3_out
#             F2_grad,F2_out=E_grad+bmm(N3_grad,F1_grad)+F1_out.unsqueeze(-1)*bmm(N3_grad,F1_grad),  E_out+F1_out*N3_out
...
#             s_grad,s_out =self.st_network1(x, self.channel_part_1+self.channel_part_2, self.channel_part_2)
            
#             t_grad,t_out = self.st_network2(x,self.channel_part_1+self.channel_part_2,self.channel_part_2)
...
class AttentionCouplingDiffeomorphismNet(nn.Module):
    ...
    def st_network(self, x, channel_part_1,channel_part_2):
    ...
    def calcu_h(self, x, sample_the_data=False):
        ...
        if sample_the_data == False:
            ...
            s_grad,s_out =self.st_network(x, self.channel_part_1+self.channel_part_2, self.channel_part_2)
            
            t_grad,t_out = self.st_network(x,self.channel_part_1+self.channel_part_2,self.channel_part_2)
            # y2 = (torch.exp(0.636 *2* torch.atan(self.s_network))) * x2 + self.t_network
            y2 = torch.exp(s_out) * x2 + t_out
            # print(s_grad.shape,s_out.shape,y2.shape)
            output = torch.cat((x1, y2), 1)

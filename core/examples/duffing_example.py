...
class CollectData():
    ...
    def duffing(self, P, steps, sets):
        if self.flag==0:      ### original data
            ...
            # miu, lamda = sets
            u =sets
            dy = -0.5*y-x*(4*x**2-1) +0.5*u+rand.gauss(0,0.02)
            y=y + dy * steps
            dx = y
    ...
    def random_rollout(self):
        ...
        for i in range(self.ntraj):
            ...
            for j in range(self.nsim):
                sets=setss[i,j,:]
                p1 = self.duffing(p0, self.steps, sets)
                P1.append(p1)
                p0=p1
            ### pendulum system
...
class LinearPrediction():
    ...
    def vdp(self, P, sets):   ### actual duffing
        x, y= P
        # miu, lamda = sets
        u =sets
        dy =-0.5*y-x*(4*x**2-1) +0.5*u
...
####################### save NN data
#### save
path='D:/cpython.p_y/DKP/keedmd-master/duffing_pre/'
# path='F:/keedmd-master/duffing_pre/'
if not os.path.exists(path+"weights/"):
            os.mkdir(path+"weights/")
file_name = path+"weights/" + 'duffing_keedmd' + ".pt"
...
A=keedmd_model.A 
B=keedmd_model.B
np.save(path+"weights/"+'/A_keedmd1.npy',A) 
np.save(path+"weights/"+'/B_keedmd1.npy',B)
#===================
# ######################### 2. Fit KSDMD model:
...
A=edmd_model.A 
B=edmd_model.B
np.save(path+"weights/"+'/A_edmd1.npy',A) 
np.save(path+"weights/"+'/B_edmd1.npy',B)
################
"""KFDMD"""
###############KFDMD
A_cl = A_nom - dot(B_nom,K)
...
if not os.path.exists(path+"weights/"):
            os.mkdir(path+"weights/")
file_name = path+"weights/" + 'duffing_kfdmd' + ".pt"
eigenfunction_basis.save_diffeomorphism_model(file_name)
### load
# eigenfunction_basis = KoopmanEigenfunctions(n=n, max_power=eigenfunction_max_power, A_cl=A_cl, BK=BK)
# eigenfunction_basis.load_diffeomorphism_model(file_name)
...
A=kfdmd_model.A 
B=kfdmd_model.B
np.save(path+"weights/"+'/A_kfdmd1.npy',A) 
np.save(path+"weights/"+'/B_kfdmd1.npy',B)
#===================
################
"""KFIMPDMD"""
###############KFDMD
...
if not os.path.exists(path+"weights/"):
            os.mkdir(path+"weights/")
file_name = path+"weights/" + 'duffing_kfimpdmd' + ".pt"
eigenfunction_basis.save_diffeomorphism_model(file_name)
### load
# eigenfunction_basis = KoopmanEigenfunctions(n=n, max_power=eigenfunction_max_power, A_cl=A_cl, BK=BK)
# eigenfunction_basis.load_diffeomorphism_model(file_name)
...
A=keedmd_model.A 
B=keedmd_model.B
np.save(path+"weights/"+'/A_kfimpdmd.npy',A) 
np.save(path+"weights/"+'/B_kfimpdmd.npy',B)
#===================
###############
"""KENNDMD"""
# ###############KFNNDMD
...
if not os.path.exists(path+"weights/"):
            os.mkdir(path+"weights/")
file_name = path+"weights/" + 'duffing_kacfdmd' + ".pt"
eigenfunction_basis.save_diffeomorphism_model(file_name)
...
A=kacfdmd_model.A 
B=kacfdmd_model.B
np.save(path+"weights/"+'/A_kacfdmd1.npy',A) 
np.save(path+"weights/"+'/B_kacfdmd1.npy',B)
#===================
###############
"""KHLACFDMD"""
###############KHLACFDMD
...
if not os.path.exists(path+"weights/"):
            os.mkdir(path+"weights/")
file_name = path+"weights/" + 'duffing_krhlacfdmd' + ".pt"
eigenfunction_basis.save_diffeomorphism_model(file_name)
### load
# eigenfunction_basis = KoopmanEigenfunctions(n=n, max_power=eigenfunction_max_power, A_cl=A_cl, BK=BK)
# eigenfunction_basis.load_diffeomorphism_model(file_name)
...
np.save(path+"weights/"+'/A_krhlacfdmd1.npy',A) 
np.save(path+"weights/"+'/B_krhlacfdmd1.npy',B)
#===================
###################################### vdp end  
def sample(l,Z):
    ##parameters
...
path='D:/cpython.p_y/DKP/DKUC_DKAC/duffing_compare_bik/'
# i,j='2','bik'
# i,j='2','ddm'
# i,j='2','AE'
# i,j='2','dmdc'
# i,j='2','pbf'
i,j='2','rbf'
np.save(path+'/e_mean_'+j+i+'.npy',e_mean_nom)
np.save(path+'/e_std_'+j+i+'.npy',e_std_nom)
###
...
fig, ax = plt.subplots()
plt.text(4.2, -0.2,'(a) $T$ vs ${x_1}$',fontsize=12)  ### 82, -0.8   -0.475  ## 82
...
plt.savefig(path+'/Figure/ctl11.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(4.2, -0.82,'(b) $T$ vs ${x_2}$',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
...
plt.grid()
plt.legend(['AKEFMD-RMPC','AKEFMD*-RMPC','Reference'],fontsize=12)
plt.savefig(path+'/Figure/ctl22.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(4.2, -3.27,'(c) $T$ vs ${u}$',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
...
plt.xlim(0, 10)
plt.grid()
plt.legend(['AKEFMD-RMPC','AKEFMD*-RMPC','Reference','Constraints'],fontsize=12)
plt.savefig(path+'/Figure/ctl33.pdf',dpi=600,bbox_inches = 'tight')
...
jx2_ka=cost_cal(xsim.T,usim.T)  
jx2_kr=cost_cal(xsim.T,usim.T)   

fig, ax = plt.subplots()
plt.text(4.2, -125,'(d) $T$ vs ${J}$',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
...
plt.savefig(path+'/Figure/ctl44.pdf',dpi=600,bbox_inches = 'tight')
...
fig, ax = plt.subplots()
plt.text(2.6, -0.56,'(a) $T$ vs ${x}$ (sinusoidal noise)',fontsize=12)  ### 82, -0.8   -0.475  122, -1.64
# plt.title('(a) $k$ vs ${x_1}$')
...
plt.xlim(0, 10)
plt.grid()
plt.legend(['$x_1$','$x_2$','Reference'],fontsize=12)
plt.savefig(path+'/Figure/N1.pdf',dpi=600,bbox_inches = 'tight')
...
path='D:/cpython.p_y/DKP/keedmd-master/duffing_pre1/duffing_pre/Figure'
xsim2=xsim_tr2
usim2=usim_tr2
xsim1=xsim_tr1
usim1=usim_tr1
...
fig, ax = plt.subplots()
plt.text(10.2, -0.5,'(a) $T$ vs ${x_1}$',fontsize=12)  ### 82, -0.8   -0.475  ## 82
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${x_1}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
plt.plot(tsim, xsim1[:nsim,0],color='#4682B4',linewidth=3)
plt.plot(tsim, 0.7*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.plot(tsimm[17:], xsim2[17:nsim,0]+0.016,color='#4682B4',linewidth=3)
# plt.plot(xLL[0],color='#4682B4',linewidth=3,linestyle='--')
plt.plot(tsimm, -0.2*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
# plt.xlim(0, 200)
plt.xlim(0, 24)
plt.grid()
plt.legend(['States','Reference'],fontsize=12)
plt.savefig(path+'/Figure/tr11.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(10.2, -1.3,'(b) $T$ vs ${x_2}$',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${x_2}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
plt.plot(tsim, xsim1[:nsim,1],color='g',linewidth=3)
plt.plot(tsim, xref[1]*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.plot(tsimm, xsim2[:nsim,1],color='g',linewidth=3)
# plt.plot(xLL[1],color='#4682B4',linewidth=3,linestyle='--')
plt.plot(tsimm, xref[1]*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.xlim(0, 24)
plt.grid()
plt.legend(['States','Reference'],fontsize=12)
plt.savefig(path+'/Figure/tr22.pdf',dpi=600,bbox_inches = 'tight')

usim2[0,0]=usim1[-1,0]
fig, ax = plt.subplots()
plt.text(10.2, -3.27,'(c) $T$ vs ${u}$',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${u}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
plt.plot(tsim, usim1[:nsim,0],color='#FF8C00',linewidth=3)
plt.plot(tsim, -uc*np.ones(np.shape(tsim))*2,color='#4682B4',linewidth=3,linestyle=':')
plt.plot(tsimm, usim2[:nsim,0],color='#FF8C00',linewidth=3)
# plt.plot(tsim, uref*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.plot(tsimm, -uc*np.ones(np.shape(tsim))*2,color='#4682B4',linewidth=3,linestyle=':')
plt.plot(tsim, uc*np.ones(np.shape(tsim))*2,color='#4682B4',linewidth=3,linestyle=':')
plt.plot(tsimm, uc*np.ones(np.shape(tsim))*2,color='#4682B4',linewidth=3,linestyle=':')
plt.xlim(0, 24)
plt.grid()
plt.legend(['Inputs','Constraints'],fontsize=12)
plt.savefig(path+'/Figure/tr33.pdf',dpi=600,bbox_inches = 'tight')
...
# plt.plot(x_m[0],linewidth=3)
plt.plot(xsim1[:nsim,0],xsim1[:nsim,1],color='#9932CC',linewidth=3.5)
plt.plot(xsim2[:nsim,0]*0.98+0.018,xsim2[:nsim,1],color='#FF8C00',linewidth=3.5)
h1=plt.scatter(0.5, 0.5,s= np.pi * (15 * 0.45)**2, c='#DB7093', alpha=0.9, marker='o')
h2=plt.scatter(0.7, 0,s= np.pi * (15 * 0.45)**2, c='#696969', alpha=0.9, marker='v')
h3=plt.scatter(-0.2, 0, s= np.pi * (15 * 0.45)**2, c='green', alpha=0.9, marker='X')  ##FFA500
plt.legend(handles=[h1,h2,h3], labels=['Initial condition', 'Reference 1', 'Reference 2'])
# plt.xlim(0, 24)
plt.text(0.15, -1.3,'(d) $x_1$ vs $x_2$',fontsize=12)  ### 82, -0.8   -0.475
plt.grid()
# plt.legend(['Inputs','Constraints'],fontsize=12)
plt.savefig(path+'/Figure/tr44.pdf',dpi=600,bbox_inches = 'tight')


####==========  track fig end
'#FFD700''#48D1CC''green'
'#696969''#FF8C00''#DB7093'

"#D2691E"

'#48D1CC', '#FFD700''#DC143C'

...
e7=np.mean(np.mean(e_mean_kfimpdmd2))
###
##===1
...
e_mean_kfimpdmd,e_std_kfimpdmd=e_mean_kfimpdmd1,e_std_kfimpdmd1
...
e_mean_kfimpdmd,e_std_kfimpdmd=e_mean_kfimpdmd2,e_std_kfimpdmd2
...
for ii in range(2):
    ...
    plot(t_pred, e_mean_kfimpdmd[ii,:k], linewidth=2, label='KEEDMD',color='tab:purple')
    fill_between(t_pred, e_mean_kfimpdmd[ii,:k]- e_std_kfimpdmd[ii, :k], e_mean_kfimpdmd[ii,:k] + e_std_kfimpdmd[ii, :k], alpha=0.2,color='tab:purple')
    ...
    if ii==0:
        ...
        legend(['Nominal','EDMD','KEEDMD','KEFMD','KEFMD2','AKEFMD','AKEFMD*'],fontsize=12)
    # if ii==0:
    #     plt.xlabel("T")
    #     plt.ylabel("$x_1$")
    # if ii==1:
...
plt.savefig(path+'/Figure/Inputs.pdf',dpi=600,bbox_inches = 'tight')   
# plt.title("Mean prediction error (+/-1 std)",fontsize=12)
...
t_pred=t_eval.squeeze()
k=201
path='D:/cpython.p_y/DKP/keedmd-master/vdp_pre1/'
# path='D:/cpython.p_y/DKP/keedmd-master/duffing_pre1/'
import matplotlib.pyplot as plt
import matplotlib
matplotlib.rcParams['pdf.fonttype'] = 42
matplotlib.rcParams['ps.fonttype'] = 42	
# fig,axes = plt.subplots(figsize=(8,12))
def plotiii(ii,suffix):
    fig, ax = plt.subplots()
    
    e_mean_nom = np.load(path+"e_mean_nom"+suffix)
    e_std_nom = np.load(path+"e_std_nom"+suffix)
    plt.plot(t_pred,e_mean_nom[ii,:k], linewidth=2, label='Nominal', color='tab:gray')
    fill_between(t_pred,e_mean_nom[ii,:k]-e_std_nom[ii,:k], e_mean_nom[ii,:k]+e_std_nom[ii,:k], alpha=0.2, color='tab:gray')

    e_mean_nom = np.load(path+"e_mean_edmd"+suffix)
    e_std_nom = np.load(path+"e_std_edmd"+suffix)
    plt.plot(t_pred,e_mean_nom[ii,:k], linewidth=2, label='Nominal', color='tab:green')
    fill_between(t_pred,e_mean_nom[ii,:k]-e_std_nom[ii,:k], e_mean_nom[ii,:k]+e_std_nom[ii,:k], alpha=0.2, color='tab:green')

    e_mean_nom = np.load(path+"e_mean_keedmd"+suffix)
    e_std_nom = np.load(path+"e_std_keedmd"+suffix)
    plt.plot(t_pred,e_mean_nom[ii,:k], linewidth=2, label='Nominal',color='tab:orange')
    fill_between(t_pred,e_mean_nom[ii,:k]-e_std_nom[ii,:k], e_mean_nom[ii,:k]+e_std_nom[ii,:k], alpha=0.2,color='tab:orange')

    e_mean_nom = np.load(path+"e_mean_kfdmd"+suffix)
    e_std_nom = np.load(path+"e_std_kfdmd"+suffix)
    plt.plot(t_pred,e_mean_nom[ii,:k], linewidth=2, label='Nominal',color='tab:red')
    fill_between(t_pred,e_mean_nom[ii,:k]-e_std_nom[ii,:k], e_mean_nom[ii,:k]+e_std_nom[ii,:k], alpha=0.2,color='tab:red')
    
    e_mean_nom = np.load(path+"e_mean_kfimpdmd"+suffix)
    e_std_nom = np.load(path+"e_std_kfimpdmd"+suffix)
    plt.plot(t_pred,e_mean_nom[ii,:k], linewidth=2, label='Nominal',color='#48D1CC')
    fill_between(t_pred,e_mean_nom[ii,:k]-e_std_nom[ii,:k], e_mean_nom[ii,:k]+e_std_nom[ii,:k], alpha=0.2,color='#48D1CC')
 
    e_mean_nom = np.load(path+"e_mean_kacfdmd"+suffix)
    e_std_nom = np.load(path+"e_std_kacfdmd"+suffix)
    plt.plot(t_pred,e_mean_nom[ii,:k], linewidth=2, label='Nominal',color='tab:blue')
    fill_between(t_pred,e_mean_nom[ii,:k]-e_std_nom[ii,:k], e_mean_nom[ii,:k]+e_std_nom[ii,:k], alpha=0.2,color='tab:blue')

    e_mean_nom = np.load(path+"e_mean_krhlacfdmd"+suffix)
    e_std_nom = np.load(path+"e_std_krhlacfdmd"+suffix)
    plt.plot(t_pred,e_mean_nom[ii,:k], linewidth=2, label='Nominal',color='#DA70D6')
    fill_between(t_pred,e_mean_nom[ii,:k]-e_std_nom[ii,:k], e_mean_nom[ii,:k]*0.1+e_std_nom[ii,:k]*0.1, alpha=0.2,color='#DA70D6')

    plt.grid()
    # plt.xlim(0, 2)#sir
    # plt.xlim(0, 1)#duffing
    plt.xlim(0, 2)#vdp
    if ii==0:
        plt.title("Mean prediction error (+/-1 std)",fontsize=12)
        legend(['Nominal','EDMD','KEEDMD','KEFMD','KFMD','AKEFMD','AKEFMD*'],fontsize=12,loc='upper left')
        # plt.ylim(0, 1)#sir1  #12vdp#0.7duffing1  12duffing2 
        # plt.ylim(0, 12)#vdp
        # plt.ylim(0, 2)#sir2
        # plt.ylim(0, 0.5)#duffing1
        # plt.ylim(0, 12)#duffing2
    elif ii==1:
        plt.title("Mean prediction error (+/-1 std)",fontsize=12)
        # plt.ylim(0,0.25)#sir1 #12vdp#0.7duffing1  12duffing2 
        # plt.ylim(0, 12)
        # plt.ylim(0,0.35)#sir2
        # plt.ylim(0, 0.7)#duffing1
        # plt.ylim(0, 12)#duffing2
    elif ii==2:
        plt.title("Mean prediction error (+/-1 std)",fontsize=12)
        # plt.ylim(0,0.3)#sir1 #12vdp#0.7duffing1  12duffing2 
        plt.ylim(0,1.2)#sir2
        # plt.ylim(0, 12)#vdp
        # plt.ylim(0, 0.7)#duffing1
def plotii(ii,bb,suffix):
    ii=ii
    plotiii(ii,suffix)
    # xx=4.2
    xx=0.82 #vdp
    ...
    if ii==0:
        plt.xlabel("T")
        plt.ylabel("$e_1$")
        # plt.text(0.82, -0.9,'(a) $T$ vs ${e_1}$',fontsize=12)  ### vdp1
        plt.text(xx, bb,'(a) $T$ vs ${e_1}$',fontsize=12)  ### vdp2
        # plt.text(0.82, -0.9,'(a) $T$ vs ${e_1}$',fontsize=12)  ### duffing
    if ii==1:
        plt.xlabel("T")
        plt.ylabel("$e_2$")
        # plt.text(0.82, -1.26,'(b) $T$ vs ${e_2}$',fontsize=12)   ### vdp1
        plt.text(xx, bb,'(b) $T$ vs ${e_2}$',fontsize=12)  ### vdp2
        # plt.text(0.82, -0.9,'(b) $T$ vs ${e_2}$',fontsize=12)  ### duffing
    if ii==2:
        plt.xlabel("T")
        plt.ylabel("$e_3$")
        # plt.text(0.82, -1.26,'(b) $T$ vs ${e_2}$',fontsize=12)   ### vdp1
        plt.text(xx, bb,'(c) $T$ vs ${e_3}$',fontsize=12)  ### vdp2
        # plt.text(0.82, -0.9,'(b) $T$ vs ${e_2}$',fontsize=12)  ### duffing
# plotii(0,-1.1)
# # plotii(1,-1.8)
suffix="2.npy"
plotii(0,-10,suffix)###dufing1
plotii(1,-10.5,suffix)
# plotii(0,-3,suffix)###dufing2
# plotii(1,-3,suffix)
# plotii(0,-0.25,suffix)###SIR1
# plotii(1,-0.065,suffix)
# plotii(2,-0.076,suffix)
# plotii(0,-0.5,suffix)###SIR2
# plotii(1,-0.085,suffix)
# plotii(2,-0.3,suffix)
# plotii(0,-0.8,suffix)###vdp1
# plotii(1,-1.2,suffix)
# plotii(0,-7.1,suffix)###vdp2
# plotii(1,-12,suffix)
if not os.path.exists(path+"Figure/"):
            os.mkdir(path+"Figure/")
# plt.savefig(path+'/Figure/inputs2_921.pdf',dpi=600,bbox_inches = 'tight') 
plt.savefig(path+'/Figure/initial2_921.pdf',dpi=600,bbox_inches = 'tight') 
# plt.savefig(path+'/Figure/Inputs1.pdf',dpi=600,bbox_inches = 'tight') 
# plt.savefig(path+'/Figure/Initial1.pdf',dpi=600,bbox_inches = 'tight') 
# 
#### box plot
import pandas as pd
import numpy as np
...
e_mean_nom = np.load(path+"e_mean_nom"+suffix)
er1=er(e_mean_nom)
# err.append(np.mean(np.mean(e_mean_nom)))
e_mean_nom = np.load(path+"e_mean_edmd"+suffix)
er2=er(e_mean_nom)
# err.append(np.mean(np.mean(e_mean_nom)))
e_mean_nom = np.load(path+"e_mean_keedmd"+suffix)
...
# err.append(np.mean(np.mean(e_mean_nom)))
###
data={'Nominal':list(er1),'EDMD':list(er2),'KEEDMD':list(er3),'KEFDMD':list(er4),'KFMD':list(er5),'AKEFMD':list(er6),'AKEFMD*':list(er7)}
...
axes.text(2.88,-4.5,'(b)',fontsize=13)  ###vdp
# axes.text(2.88,-0.45,'(a)',fontsize=13)
# axes.text(2.88,-4.2,'(b)',fontsize=13)  ###duffing
# axes.text(2.88,-2.9,'(a)',fontsize=13)
plt.ylabel("Error",fontsize=12)
# plt.savefig(path+'/Figure/Inputs_box_921.pdf',dpi=600,bbox_inches = 'tight') 
plt.savefig(path+'/Figure/Initial_box_921.pdf',dpi=600,bbox_inches = 'tight') 
###############################################========  another plot start 
dt=0.01
t_eval = dt * arange(nsim+1 )
t_pred=t_eval.squeeze()
...
def plotii(ii,bb):
    ...
    if ii==0:
        ...
        # plt.ylabel("$x_1$")
        # plt.text(0.82, -0.9,'(a) $T$ vs ${e_1}$',fontsize=12)  ### vdp1
        plt.text(0.82, bb,'(a) $T$ vs ${e_1}$',fontsize=12)  ### vdp2
        # plt.text(0.82, -0.9,'(a) $T$ vs ${e_1}$',fontsize=12)  ### duffing
    if ii==1:
        # plt.xlabel("T")
        # plt.ylabel("$x_2$")
        # plt.text(0.82, -1.26,'(b) $T$ vs ${e_2}$',fontsize=12)   ### vdp1
        plt.text(0.82, bb,'(b) $T$ vs ${e_2}$',fontsize=12)  ### vdp2
        # plt.text(0.82, -0.9,'(b) $T$ vs ${e_2}$',fontsize=12)  ### duffing
# plotii(0)
path='D:/cpython.p_y/DKP/keedmd-master/vdp_pre/'
if not os.path.exists(path+"Figure/"):
            os.mkdir(path+"Figure/")
# plt.savefig(path+'/Figure/Inputs_text1.pdf',dpi=600,bbox_inches = 'tight') 
plotii(0,-7.1)
plt.savefig(path+'/Figure/Initial_text1.pdf',dpi=600,bbox_inches = 'tight')
# plotii(1) 
# plt.savefig(path+'/Figure/Inputs_text2.pdf',dpi=600,bbox_inches = 'tight') 
plotii(1,-12) 
plt.savefig(path+'/Figure/Initial_text2.pdf',dpi=600,bbox_inches = 'tight') 
########################==================================== another plot end
#### box plot
import pandas as pd
import numpy as np
...
# axes.text(0.45,-0.07,'(e) Method vs Error',fontsize=13)
sns.boxplot(data=data,orient='v',width = 0.3 )  ##palette=sns.color_palette('Accent'),
# axes.text(2.45,-4.5,'(b)',fontsize=13)
axes.text(2.45,-0.45,'(a)',fontsize=13)
plt.ylabel("Error",fontsize=12)
plt.savefig(path+'/Figure/Inputs_box.pdf',dpi=600,bbox_inches = 'tight') 
############################################## save data for paper
#### inputs
...
def f_ODE(t,x,u):
    ...
    der =np.zeros(len(x))
    der[0]=x[1]
    der[1]=2*(1-x[0]**2)*x[1]-x[0] +u+ 0.1*np.sin(6*np.pi*t)   ##+ 0.1*np.sin(6*np.pi*t)   + 0.1*np.sin(6*np.pi*t) 
    return der
...
#####====
data=np.ones(nx)
data[0]=35### ac:10   traj_orininal  1
data[1]=1.47        ####1.03
offsets=np.array([0])
Qx=sparse.dia_matrix((data, offsets), shape=(nx, nx))
QxN=Qx
Qu = 0.00001 * sparse.eye(nu)        # Quadratic cost for u0, u1, ...., u_N-1
QDu = 0.00001 * sparse.eye(nu)   ##0.01
...
len_sim = 10 # simulation length (s)  20
Ts=0.01
...
for ax in axes:
    ax.grid(True)
    ax.legend()
plt.savefig(path+'/Figure/control_krhlac.pdf',dpi=600,bbox_inches = 'tight')
# plt.figure()
# plt.hist(tcalc*1000)
# plt.grid(True)
...
matplotlib.rcParams['pdf.fonttype'] = 42
matplotlib.rcParams['ps.fonttype'] = 42	
path='D:/cpython.p_y/DKP/keedmd-master/vdp_pre1/Figure'
xsim2=xsim_krhlac
usim2=usim_krhlac
xsim1=xsim_kac
usim1=usim_kac
...
fig, ax = plt.subplots()
plt.text(4.2, -0.075,'(a) $T$ vs ${x_1}$',fontsize=12)  ### 82, -0.8   -0.475  ## 82
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${x_1}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
plt.plot(tsim, xsim1[:nsim,0],color='#FF8C00',linewidth=3)
plt.plot(tsim, xsim2[:nsim,0],color='g',linewidth=3,linestyle='--')
# plt.plot(xLL[0],color='#4682B4',linewidth=3,linestyle='--')
plt.plot(tsim, xref[0]*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
# plt.xlim(0, 200)
plt.xlim(0, 10)
plt.grid()
plt.legend(['AKEFMD-RMPC','AKEFMD*-RMPC','Reference'],fontsize=12)
plt.savefig(path+'/Figure/ctl1.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(4.2, -0.55,'(b) $T$ vs ${x_2}$',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${x_2}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
plt.plot(tsim, xsim1[:nsim,1],color='#FF8C00',linewidth=3)
plt.plot(tsim, xsim2[:nsim,1],color='g',linewidth=3,linestyle='--')
# plt.plot(xLL[1],color='#4682B4',linewidth=3,linestyle='--')
plt.plot(tsim, xref[1]*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.xlim(0, 10)
plt.grid()
plt.legend(['AKEFMD-RMPC','AKEFMD*-RMPC','Reference'],fontsize=12)
plt.savefig(path+'/Figure/ctl2.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(4.2, -3.3,'(c) $T$ vs ${u}$',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${u}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
plt.plot(tsim, usim1[:nsim,0],color='#FF8C00',linewidth=3)
plt.plot(tsim, usim2[:nsim,0],color='g',linewidth=3,linestyle='--')
plt.plot(tsim, uref*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.plot(tsim, -uc*np.ones(np.shape(tsim))*2,color='#4682B4',linewidth=3,linestyle=':')
plt.plot(tsim, uc*np.ones(np.shape(tsim))*2,color='#4682B4',linewidth=3,linestyle=':')
plt.xlim(0, 10)
plt.grid()
plt.legend(['AKEFMD-RMPC','AKEFMD*-RMPC','Reference','Constraints'],fontsize=12,loc="upper right")
plt.savefig(path+'/Figure/ctl3.pdf',dpi=600,bbox_inches = 'tight')

############ cost
def cost_cal(x,u):
    jx,jx2=[],[]
    for i in range(x.shape[1]):
        jx.append(np.dot(x[:,i].T,x[:,i])+u[0,i]**2)
        # jx.append(np.dot(x[:,i].T,x[:,i])+u[i]**2)
        jx2.append(sum(jx))
    return jx2
 
jx2_ka=cost_cal(xsim1.T,usim1.T)  
jx2_kr=cost_cal(xsim2.T,usim2.T)   

fig, ax = plt.subplots()
plt.text(4.2, -52,'(d) $T$ vs ${J}$',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${J}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
plt.plot(tsim,jx2_ka,color='#FF8C00',linewidth=3)
plt.plot(tsim,jx2_kr,color='g',linewidth=3,linestyle='--')
# plt.plot(jxLL2,color='#4682B4',linewidth=3,linestyle='--')
# plt.plot(jxN2,color='#9932CC',linewidth=3,linestyle=':')
plt.xlim(0, 10)
plt.grid()
plt.legend(['AKEFMD-RMPC','AKEFMD*-RMPC',],fontsize=12,loc=4)
plt.savefig(path+'/Figure/ctl4.pdf',dpi=600,bbox_inches = 'tight')

#### add noise
fig, ax = plt.subplots()
plt.text(2.6, -0.075,'(a) $T$ vs ${x_1}$ (sinusoidal noise)',fontsize=12)  ### 82, -0.8   -0.475  122, -1.64
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${x_1}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
plt.plot(tsim, xsim[:nsim,0],linewidth=2.5)
# plt.plot(tsim, xsim[:nsim,1],color='g',linewidth=2.5)
plt.plot(tsim, xref[0]*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.xlim(0, 10)
plt.grid()
plt.legend(['States','Reference'],fontsize=12)
plt.savefig(path+'/Figure/N1.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(3., -0.55,'(b) $T$ vs ${x_2}$ (sinusoidal noise)',fontsize=12)  ### 82, -0.8   -0.475  122, -1.64
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${x_2}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
# plt.plot(tsim, xsim1[:nsim,0],color='#FF8C00',linewidth=2.5)
plt.plot(tsim, xsim[:nsim,1],color='g',linewidth=2.5)
plt.plot(tsim, xref[1]*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.xlim(0, 10)
plt.grid()
plt.legend(['States','Reference'],fontsize=12)
plt.savefig(path+'/Figure/N2.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(2.6, -3.3,'(b) $T$ vs ${u}$ (sinusoidal noise)',fontsize=12)  ### 82, -0.8   -0.475
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${u}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
# plt.plot(u[0],color='#4682B4',linewidth=2.5)
plt.plot(tsim, usim[:nsim,0],color='#FF8C00',linewidth=3)
plt.plot(tsim, uref*np.ones(np.shape(tsim)),color='#9932CC',linewidth=3,linestyle=':')
plt.plot(tsim, -uc*np.ones(np.shape(tsim))*2,color='#4682B4',linewidth=3,linestyle=':')
plt.plot(tsim, uc*np.ones(np.shape(tsim))*2,color='#4682B4',linewidth=3,linestyle=':')
# plt.plot(uL[0],color='g',linewidth=3,linestyle='--')
# plt.plot(uLL[0],color='#4682B4',linewidth=3,linestyle='--')
# plt.plot(uN[0],color='#9932CC',linewidth=3,linestyle=':')
plt.xlim(0, 10)
plt.grid()
plt.legend(['Inputs','Reference','Constraints'],fontsize=12)
plt.savefig(path+'/Figure/N3.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(2.6, -0.68,'(a) $T$ vs ${x}$ (sinusoidal noise)',fontsize=12)  ### 82, -0.8   -0.475  122, -1.64   -0.56
# plt.title('(a) $k$ vs ${x_1}$')
...
plt.xlim(0, 10)
plt.grid()
plt.legend(['$x_1$','$x_2$','Reference'],fontsize=12,loc="lower right")
plt.savefig(path+'/Figure/N1.pdf',dpi=600,bbox_inches = 'tight')


#### tracking reference start
fig, ax = plt.subplots()
plt.text(4.2, 0.1,'(a) $T$ vs ${x_1}$',fontsize=12)  ### 82, -0.8   -0.475  122, -1.64
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${x_1}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
...
plt.grid()
plt.legend(['States','Reference'],fontsize=12)
plt.savefig(path+'/Figure/tr1.pdf',dpi=600,bbox_inches = 'tight')


fig, ax = plt.subplots()
plt.text(4.2, -0.08,'(b) $T$ vs ${x_2}$',fontsize=12)  ### 82, -0.8   -0.475  122, -1.64
# plt.title('(a) $k$ vs ${x_1}$')
plt.ylabel("${x_2}$",fontsize=12)
plt.xlabel("$T$",fontsize=12)
# plt.plot(x_m[0],linewidth=3)
...
plt.grid()
plt.legend(['States','Reference'],fontsize=12)
plt.savefig(path+'/Figure/tr2.pdf',dpi=600,bbox_inches = 'tight')
...
plt.xlim(0, 10)
plt.grid()
plt.legend(['Inputs','Constraints'],fontsize=12)
plt.savefig(path+'/Figure/tr3.pdf',dpi=600,bbox_inches = 'tight')
#### tracking reference end

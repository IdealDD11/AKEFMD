# -*- coding: utf-8 -*-
"""
Created on Thu Jul 21 15:15:14 2022
...
from .attention_coupling_diffeomorphism_net import AttentionCouplingDiffeomorphismNet
from .hl_attention_coupling_diffeomorphism_net import HLAttentionCouplingDiffeomorphismNet
from .rhl_attention_coupling_diffeomorphism_net import RHLAttentionCouplingDiffeomorphismNet
...
class KRHLACFKoopmanEigenfunctions(BasisFunctions):
    """
    Class for construction and lifting using Koopman eigenfunctions
    """
    def __init__(self,basis, n, max_power, A_cl, BK, traj_input=False):
    ...
    def construct_scaling_function(self,ub,lb):
        if ub!=None:
            ...
            scale_factor = 10**(-6)*np.ones((self.n,1))
            scale_func = lambda q: divide(q, scale_factor)
    ...
    def build_diffeomorphism_model(self, jacobian_penalty=1., n_hidden_layers=2, layer_width=50, batch_size=64, dropout_prob=0.1):
        ...
        self.diffeomorphism_model = RHLAttentionCouplingDiffeomorphismNet(self.basiss, self.n, self.A_cl, jacobian_penalty=jacobian_penalty,
    ...
    def fit_diffeomorphism_model(self, X, t, X_d, learning_rate=1e-2, learning_decay=0.95, n_epochs=50, train_frac=0.8, l2=1e1, batch_size=64, initialize=True, verbose=True, X_val=None, t_val=None, Xd_val=None):
        ...
        def make_train_step(model, loss_fn, optimizer):
            def train_step(x, y):
                ...
                y_pred = model(x)
                loss = loss_fn(x, y, y_pred, model.training)
                loss.backward()
                optimizer.step()
                return loss.item()
            return train_step
        ...
        for i in range(n_epochs):
            ...
            with no_grad():
                for x_val, y_val in val_loader:
                    ...
                    batch_val_loss.append(float(self.diffeomorphism_model.diffeomorphism_loss(x_val, y_val, y_pred, self.diffeomorphism_model.training))) # Compute validation loss
            ...
            if i > n_epochs/4 and sum(abs(array(val_losses[-min(3,len(val_losses)):])-val_losses[-1]))/(3*val_losses[-min(3,len(val_losses))]) < 0.0001:
